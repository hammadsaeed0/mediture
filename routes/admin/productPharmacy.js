...
router.post("/new", validateInputs(["image", "title", "price", "description", "pharmacy", "packSize", "discount"]), async (req, res) => {
    try {
        let { image, title, price, description, pharmacy, packSize, discount } = req.body;
        const promoCodeDB = await Model.create({
            image, title, price, description, pharmacy, packSize, discount
        });
...
router.post("/edit", validateInputs(["id", "image", "title", "price", "description", "pharmacy", "packSize", "discount"]), async (req, res) => {
    try {
        let { image, price, description, title, id, pharmacy, packSize, discount } = req.body;

...
const Banner = require("../../models/bannerHome");
const { validateInputs } = require("../../functions/validate");
const { upload } = require("../../functions/upload");

router.post("/new", upload.single("image"), async (req, res) => {
    try {
        const bannerHome = await Banner.create({
            image: newFileName,
        });
...
router.post("/edit", validateInputs(["id"]), upload.single("image"), async (req, res) => {
    try {
        const bannerHome = await Banner.findByIdAndUpdate(id, {
            image: newFileName,
        });
...
router.post("/delete", validateInputs(["id"]), async (req, res) => {
    try {
        let { id } = req.body;
        const bannerHome = await Banner.findByIdAndDelete(id);
        return res.status(200).json({
            status: "success",
        });
    } catch (err) {
...
router.get("/:id", async (req, res) => {
    try {
        const bannerHome = await Banner.findById(req.params.id);
        return res.status(200).json({
            status: "success",
            data: bannerHome
        });
